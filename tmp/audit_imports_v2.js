...
const uiComponents = new Set(['Card', 'CardHeader', 'CardTitle', 'CardDescription', 'CardContent', 'CardFooter', 'Button', 'Input', 'Label', 'Tabs', 'TabsList', 'TabsTrigger', 'TabsContent', 'Select', 'SelectTrigger', 'SelectValue', 'SelectContent', 'SelectItem', 'Dialog', 'DialogContent', 'DialogDescription', 'DialogHeader', 'DialogTitle', 'DialogTrigger', 'Badge', 'Calendar', 'Avatar', 'AvatarImage', 'AvatarFallback', 'Switch', 'Checkbox', 'RadioGroup', 'RadioGroupItem', 'Separator', 'Skeleton', 'ScrollArea', 'Accordion', 'AccordionItem', 'AccordionTrigger', 'AccordionContent', 'AlertDialog', 'AspectRatio', 'Collapsible', 'ContextMenu', 'HoverCard', 'Menubar', 'NavigationMenu', 'Popover', 'Progress', 'Slider', 'Toast', 'Tooltip', 'Sheet', 'Table', 'TableHeader', 'TableBody', 'TableFooter', 'TableHead', 'TableRow', 'TableCell', 'TableCaption', 'Textarea']);
...
files.forEach(file => {
    ...
    const importedIconsSet = new Set();
    if (lucideMatch) {
        lucideMatch[1].split(',').forEach(s => importedIconsSet.add(s.trim()));
    }
    ...
    usedComponents.forEach(comp => {
        if (!standardTags.has(comp.toLowerCase()) && !uiComponents.has(comp) && !importedIconsSet.has(comp)) {
            ...
            const isImported = new RegExp(`import .*\\b${comp}\\b.* from`).test(content) || 
                               new RegExp(`const .*\\b${comp}\\b.* =`).test(content) ||
                               new RegExp(`function \\b${comp}\\b`).test(content);
            if (!isImported) {
                console.log(`[MISSING IMPORT] ${comp} in ${file}`);
            }
        }
    });
});
