...
export default function App() {
  ...
  const renderContent = () => {
    ...
    // 2. Post-registration onboarding screen
    ...
    // 3. Dashboard routing
    if (user && profile) {
      const Dashboard = profile.role === 'admin' ? AdminDashboard : 
                        profile.role === 'trainer' ? TrainerDashboard : 
                        MemberDashboard;
    ...
    // 4. Login/Register screen (Default)
    return (
      <div className="min-h-screen bg-[#030303] relative overflow-hidden">
