...
export async function authFetch(path, options = {}) {
  ...
  let res = await fetch(getApiUrl() + path, { ...options, headers: buildHeaders(session?.access_token) })

  // Si el servidor rechaza el token, intentar refrescar la sesión y reintentar una vez
  if (res.status === 401) {
    try {
      const { data: { session: fresh } } = await supabase.auth.refreshSession()
      if (fresh?.access_token) {
        res = await fetch(getApiUrl() + path, { ...options, headers: buildHeaders(fresh.access_token) })
      }
    } catch (refreshErr) {
      console.warn('authFetch: error al refrescar sesión:', refreshErr?.message)
    }
  }
