...
/**
 * Cumplimiento REAL de pasos en el periodo que cubre una revisión.
 *
 * Distinto de gatherActivityContext: aquí no interesa el gasto sino contrastar
 * lo que el socio DICE en el formulario ("¿has cumplido los pasos?") con lo que
 * realmente hizo. Se usa al enviar la revisión y se guarda como snapshot.
 *
 * @param {string|null} sinceDate  Fecha ISO (YYYY-MM-DD) de la revisión anterior.
 *                                 Sin ella se usan los últimos `fallbackDays` días.
 * @returns {Promise<object|null>} null si no hay ni un solo día con pasos.
 */
export async function gatherStepsCompliance(supabase, memberId, { sinceDate = null, fallbackDays = 28 } = {}) {
  if (!memberId) return null

  try {
    const DAY_MS = 24 * 60 * 60 * 1000
    const fallbackFrom = isoDay(new Date(Date.now() - (fallbackDays - 1) * DAY_MS))
    // Si la revisión anterior es muy antigua, no tiene sentido juzgar seis meses
    // de golpe: se acota a la ventana de respaldo.
    const from = (sinceDate && sinceDate > fallbackFrom) ? sinceDate : fallbackFrom
    const today = isoDay(new Date())

    const [{ data: profile }, { data: rows }] = await Promise.all([
      supabase.from('profiles').select('steps_goal').eq('id', memberId).maybeSingle(),
      supabase
        .from('daily_activity')
        .select('activity_date, steps, source')
        .eq('member_id', memberId)
    ...
    const stepsGoal = Number(profile?.steps_goal) || 8000
    const valid = (rows || []).filter(r => Number(r.steps) > 0)
    if (valid.length === 0) return null

    // Días naturales que abarca el periodo (incluyendo ambos extremos).
    const periodDays = Math.max(
      1,
      Math.round((new Date(today) - new Date(from)) / DAY_MS) + 1
    )

    const daysWithData = valid.length
    const daysGoalMet = valid.filter(r => Number(r.steps) >= stepsGoal).length
    const avgSteps = Math.round(valid.reduce((s, r) => s + Number(r.steps), 0) / daysWithData)
    const deviceDays = valid.filter(r => r.source === 'device').length
    ...
    return {
      ...
      compliance_pct: Math.round((daysGoalMet / daysWithData) * 100),
      device_days: deviceDays,
      // Con registro en menos de la mitad de los días, el porcentaje es una
      // anécdota: puede haber sincronizado solo los días que anduvo mucho.
      reliable: daysWithData >= Math.ceil(periodDays / 2),
    }
  } catch (e) {
    console.warn('gatherStepsCompliance error:', e.message)
    return null
  }
}
