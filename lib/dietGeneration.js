...
- NO incluyas suplementación detallada ni descargo de responsabilidad — eso se añade automáticamente al final del documento.`
  ...
  // Detecta el rechazo de seguridad del modelo (español e inglés). Como la
  // respuesta no es determinista (temperature > 0), un mismo perfil puede
  ...
  // Reintento automático ante rechazos: hasta 3 intentos. Subimos ligeramente la
  // temperatura en cada reintento para forzar una respuesta distinta del modelo.
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    rawMealsText = aiResponse.choices[0]?.message?.content || ''

    if (!refusalPatterns.test(rawMealsText.trim())) break

    console.warn(`generateNLEliteDiet: rechazo del modelo en intento ${attempt}/${MAX_ATTEMPTS}: "${rawMealsText.slice(0, 120)}"`)
...
9. MUY IMPORTANTE: tu respuesta debe empezar DIRECTAMENTE por el documento (la primera línea debe ser "## DISTRIBUCIÓN DE COMIDAS" o "# PROGRAMA NUTRICIONAL"). NO escribas ningún preámbulo, título, línea de "═══", "DIETA ADAPTADA DE...", ni comentario antes ni después del documento. Solo el documento.`
  ...
  const aiResponse = await openai.chat.completions.create({
    model: 'gpt-4o',
    messages: [
      { role: 'system', content: 'Eres un dietista-nutricionista deportivo colegiado. Adaptas dietas ya existentes de socios reales a partir de sus revisiones periódicas. SIEMPRE respondes en español y con el documento completo.' },
      { role: 'user', content: prompt }
    ],
    max_tokens: 4000,
    temperature: 0.4,
  ...
  const adaptedContent = stripDietPreamble(aiResponse.choices[0]?.message?.content?.trim()) || currentDietContent
...
5. Responde ÚNICAMENTE con un JSON con esta forma exacta, sin texto ni markdown antes o después:
{"content": "<documento completo corregido>", "calories": <number>, "protein_g": <number>, "carbs_g": <number>, "fat_g": <number>, "explanation": "<respuesta directa y justificación nutricional para el entrenador sobre lo que se ha hecho o la pregunta planteada>"}`
