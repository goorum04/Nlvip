...
export async function generateNLEliteDiet({
  ...
  const [{ data: profile }, dietContext, { data: onboarding }, aversionResult, memberNotesResult, adminPrefsResult, activityContext] = await Promise.all([
    supabase
      .from('profiles')
...
ANÁLISIS DE LA IA SOBRE SUS FOTOS DE PROGRESO (comparadas con la revisión anterior):
"${photoAnalysis || 'sin fotos comparables aún'}"
${memoryBlock}
═══════════════════════════════════════
...
ANÁLISIS DE FOTOS: "${photoAnalysis || 'ninguno'}"
...
export async function refineDietDraft({ currentContent, currentMacros = {}, correction, supabase = null, memberId = null }) {
  ...
  let physicalBlock = ''
  if (supabase) {
    const [memberNotesResult, adminPrefsResult, profileResult] = await Promise.all([
      memberId
        ? Promise.resolve(supabase.rpc('rpc_get_member_notes_text', { p_member_id: memberId })).catch((e) => { console.warn('rpc_get_member_notes_text falló:', e?.message); return { data: '' } })
