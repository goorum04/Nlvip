...
export async function generateNLEliteDiet({
  ...
  // 1. Perfil del socio + historial de dieta + formulario de onboarding en paralelo
  const [{ data: profile }, dietContext, { data: onboarding }] = await Promise.all([
    supabase
      .from('profiles')
  ...
  // Fusionar restricciones y condiciones: parámetro > formulario > perfil
  ...
  const restrictions = [restrictionsParam, formRestrictions, profileAllergies].filter(Boolean).join(', ') || 'ninguna'
...
- NO incluyas suplementación detallada ni descargo de responsabilidad — eso se añade automáticamente al final del documento.`
  ...
  const aiResponse = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: 'Eres un dietista y preparador físico profesional especializado en nutrición deportiva y planificación de dietas personalizadas. Tu función es crear planes alimentarios detallados, seguros y saludables para clientes de gimnasio. Responde siempre en español.'
      },
