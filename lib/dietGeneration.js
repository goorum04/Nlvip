...
export async function generateNLEliteDiet({
  ...
  const [{ data: profile }, dietContext, { data: onboarding }, aversionResult, memberNotesResult, adminPrefsResult] = await Promise.all([
    supabase
      .from('profiles')
...
export async function adaptExistingDiet({
  ...
  const [{ data: profile }, dietContext, memberNotesResult, adminPrefsResult] = await Promise.all([
    supabase
      .from('profiles')
      .select('name, email, weight_kg, height_cm, age, sex, goal, activity_level, allergies, injuries, medical_conditions')
      .eq('id', memberId)
