...
export async function refineDietDraft({ currentContent, currentMacros = {}, correction, supabase = null, memberId = null }) {
  ...
  const prompt = `Eres un nutricionista deportivo experto del club NL VIP. El entrenador está revisando un borrador de dieta ya adaptado tras una revisión periódica y quiere aplicar una corrección puntual.
...
2. Ajusta gramos de otros alimentos si hace falta para mantener coherencia nutricional, salvo que la corrección pida explícitamente cambiar los macros totales.
3. Actualiza la sección "## TUS MACROS DIARIOS" con los macros totales NUEVOS resultantes.${directionInstruction}
5. Responde ÚNICAMENTE con un JSON con esta forma exacta, sin texto ni markdown antes o después:
{"content": "<documento completo corregido>", "calories": <number>, "protein_g": <number>, "carbs_g": <number>, "fat_g": <number>}`
...
Devuelve 2-5 bullets muy breves, en primera persona (uso interno para el entrenador, el socio nunca los ve), explicando qué cambiaste exactamente y cómo responde a su petición. Empieza cada uno con "• ". Sin título ni despedida.`
  ...
  return { content, macros: newMacros, changeSummary }
}
