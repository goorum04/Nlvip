...
export async function adaptExistingDiet({
  ...
  const age = profile?.age || 30
  const sex = profile?.sex || 'hombre'
  const resolvedGoal = profile?.goal || 'maintain'
  const calculatorGoal = goalFromOnboarding(resolvedGoal)
  const calculatorActivity = profile?.activity_level
    ? activityFromWorkIntensity(profile.activity_level)
    : 'moderate'

  const recalculated = calculateMacros({
    weight, height, age, sex,
    activity: calculatorActivity,
    goal: calculatorGoal,
  })
  const newMacros = recalculated || {
    calories: currentMacros.calories || 2000,
    protein_g: currentMacros.protein_g || 150,
    carbs_g: currentMacros.carbs_g || 200,
    fat_g: currentMacros.fat_g || 60,
  }
...
═══════════════════════════════════════
- Peso actual: ${weight}kg | Macros anteriores: ${currentMacros.calories || '?'} kcal | P:${currentMacros.protein_g || '?'}g | HC:${currentMacros.carbs_g || '?'}g | G:${currentMacros.fat_g || '?'}g
- Macros objetivo NUEVOS (recalculados con el peso actual): ${newMacros.calories} kcal | P:${newMacros.protein_g}g | HC:${newMacros.carbs_g}g | G:${newMacros.fat_g}g
...
7. Devuelve el DOCUMENTO COMPLETO adaptado (todas las secciones, de principio a fin), no solo la parte que cambia. No incluyas explicaciones fuera del documento.`
  ...
  const adaptedContent = aiResponse.choices[0]?.message?.content?.trim() || currentDietContent
