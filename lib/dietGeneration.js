...
export async function generateNLEliteDiet({
  ...
  // 1. Perfil del socio + historial de dieta en paralelo
  ...
  const weight = (weightParam && isFinite(weightParam) && weightParam > 0) ? weightParam : (profile?.weight_kg || 75)
  const height = (heightParam && isFinite(heightParam) && heightParam > 0) ? heightParam : (profile?.height_cm || 170)
  ...
  // Actividad: parámetro explícito > intensidad del trabajo > activity_level del perfil
  let calculatorActivity = 'moderate'
  if (intensidadTrabajo) {
    calculatorActivity = activityFromWorkIntensity(intensidadTrabajo)
  ...
  // Fusionar restricciones y condiciones del parámetro + perfil
  const profileAllergies = profile?.allergies || ''
  const profileMedical = profile?.medical_conditions || ''
  const restrictions = [restrictionsParam, profileAllergies].filter(Boolean).join(', ') || 'ninguna'
  const medConditions = [medConditionsParam, profileMedical].filter(Boolean).join('; ') || 'ninguna'
...
- Preferencias: ${preferences || 'omnívoro'}
...
- NO incluyas suplementación detallada ni descargo de responsabilidad — eso se añade automáticamente al final del documento.`
  ...
  // Elimina el checklist de verificación interna antes de que llegue al socio.
  const mealsText = stripInternalChecklist(rawMealsText)

  // 7. Envolver con la plantilla oficial NL VIP
...
PERFIL: ${memberName}, ${sex}, ${age} años, ${weight}kg. Objetivo: ${goal}. ${numMeals} comidas/día.
Macros objetivo: ${calories} kcal | P:${protein_g}g | HC:${carbs_g}g | G:${fat_g}g.
