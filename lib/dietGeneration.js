...
export async function generateNLEliteDiet({
  ...
  const [{ data: profile }, dietContext, { data: onboarding }, aversionResult, activityContext] = await Promise.all([
    supabase
      .from('profiles')
...
export async function adaptExistingDiet({
  ...
  const [{ data: profile }, dietContext, activityContext] = await Promise.all([
    supabase
      .from('profiles')
      .select('name, email, weight_kg, height_cm, age, sex, goal, activity_level, allergies, injuries, medical_conditions')
      .eq('id', memberId)
...
═══════════════════════════════════════
INSTRUCCIONES — ADAPTAR, NO REESCRIBIR
═══════════════════════════════════════
1. Parte del documento actual tal cual. Mantén la MISMA estructura, el mismo número de comidas, los mismos horarios y, en la medida de lo posible, las mismas comidas y opciones — esto NO es una dieta nueva.
2. Ajusta ÚNICAMENTE lo que sea necesario para llevar el total diario a los macros objetivo NUEVOS (±5%): tamaños de ración (gramos), y solo si hace falta, sustituye algún alimento puntual por otro nutricionalmente equivalente.
3. Si las notas o el análisis de fotos indican algo relevante (hinchazón/retención, digestión mala, fatiga, poca adherencia, buena evolución, etc.), aplica el ajuste de sentido común correspondiente (p.ej. bajar sodio/lácteos si hay hinchazón, simplificar si hay poca adherencia) — pero sigue sin inventar una dieta distinta.
4. Actualiza la sección "## TUS MACROS DIARIOS" con los macros objetivo NUEVOS.
5. Conserva EXACTAMENTE el formato de cada sección: encabezados "## NOMBRE SECCIÓN" en mayúsculas tal cual aparecen en el documento actual (## DISTRIBUCIÓN DE COMIDAS, ## REGLAS GENERALES PARA TODAS LAS COMIDAS, ## SUPLEMENTACIÓN, ## OBSERVACIONES GENERALES, ## FLUIDOS), las líneas de comida con el formato "<emoji> NOMBRE (HH:MM):", las opciones con "- Opción N: ...", y el disclaimer final que empieza por "⚠️ DESCARGO DE RESPONSABILIDAD". NO cambies estos encabezados ni el formato, solo el contenido dentro de ellos cuando corresponda.
6. Si una sección (reglas, suplementación, observaciones, fluidos, disclaimer) no necesita cambios, cópiala EXACTAMENTE igual que en el documento actual.
7. Devuelve el DOCUMENTO COMPLETO adaptado (todas las secciones, de principio a fin), no solo la parte que cambia.
8. MUY IMPORTANTE: tu respuesta debe empezar DIRECTAMENTE por el documento (la primera línea debe ser "## DISTRIBUCIÓN DE COMIDAS" o "# PROGRAMA NUTRICIONAL"). NO escribas ningún preámbulo, título, línea de "═══", "DIETA ADAPTADA DE...", ni comentario antes ni después del documento. Solo el documento.`
...
export async function refineDietDraft({ currentContent, currentMacros = {}, correction }) {
  const openai = getOpenAI()
  const baseMacros = {
