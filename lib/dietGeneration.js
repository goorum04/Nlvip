...
- NO incluyas suplementación detallada ni descargo de responsabilidad — eso se añade automáticamente al final del documento.`
  ...
  const systemMsg = {
    role: 'system',
    content: 'Eres un dietista y preparador físico profesional especializado en nutrición deportiva. Trabajas en un gimnasio y tu tarea es crear planes de alimentación personalizados y seguros para adultos sanos que entrenan. Siempre generas dietas completas en español siguiendo las instrucciones del preparador.'
  }
  const refusalPattern = /^(lo siento|no puedo|no es posible|no me es posible|disculpa|perdona|lamentablemente|i('m| am) (sorry|unable)|i cannot|i can't)/i

  const callOpenAI = (userContent) =>
  ...
  let aiResponse = await callOpenAI(prompt)
  let rawMealsText = aiResponse.choices[0]?.message?.content || ''

  // Si el modelo rechaza, reintentar reemplazando los campos de datos del socio
  // que podrían contener términos sensibles (condición médica, lesiones, aversiones)
  if (refusalPattern.test(rawMealsText.trim())) {
    console.warn('generate-diet: modelo rechazó con campos originales, reintentando con campos sanitizados')
    const safePrompt = prompt
      .replace(/condición médica \([^)]*\)/gi, 'condición médica (ninguna relevante para la dieta)')
      .replace(/lesiones \([^)]*\)/gi, 'lesiones (ninguna relevante para la dieta)')
      .replace(/restricciones\/alergias \([^)]*\)/gi, 'restricciones/alergias (ninguna)')
    aiResponse = await callOpenAI(safePrompt)
    rawMealsText = aiResponse.choices[0]?.message?.content || ''
  }

  // Si sigue rechazando tras el reintento, lanzar error descriptivo
  if (refusalPattern.test(rawMealsText.trim())) {
