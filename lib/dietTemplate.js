...
• **IMPORTANTE:** Para mejorar la absorción de nutrientes, evita tomar muchos líquidos durante las comidas.
  ...
  generateFluidsSection: function({ weight, sex, goal, activityLevel, trainTime }) {
    ...
    // Mínimo fisiológico: 2.0L mujeres, 2.5L hombres
    const minimumMl = isFemale ? 2000 : 2500

    const baseTotal = Math.max(minimumMl, Math.round((w * base * sexFactor + goalExtra) / 100) * 100)
    const trainTotal = Math.max(minimumMl, Math.round((baseTotal + trainBonus) / 100) * 100)
