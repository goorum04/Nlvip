...
// Sonnet 5 para el diseño y formateo de rutinas (igual que dietGeneration.js);
// Haiku 4.5 para el resumen interno corto (rationale/changeSummary), que
// nadie salvo el entrenador/admin ve.
const CLAUDE_MODEL = 'claude-sonnet-5'
const CLAUDE_MODEL_FAST = 'claude-haiku-4-5'
// Sonnet 5 no acepta temperature/top_p/top_k (400). El thinking adaptativo se
// desactiva: el prompt ya es muy prescriptivo (reglas numeradas, plantillas
// de split exactas), no hace falta razonamiento extra y así max_tokens no
// compite con el pensamiento.
...
// Esquema estricto para el formateo de la rutina a JSON — sustituye al
// response_format:json_object de OpenAI (que solo "pedía" JSON por prompt).
// Con json_schema Claude garantiza la forma exacta, incluidos los nombres de
// campo que persistRoutine/enforceCatalogNames necesitan.
const ROUTINE_SCHEMA = {
  type: 'object',
  properties: {
    routine_name: { type: 'string' },
    routine_description: { type: 'string' },
    medical_rationale: { type: 'string' },
    days: {
      type: 'array',
      items: {
        type: 'object',
        properties: {
          ...
          exercises: {
            ...
            items: {
              ...
              required: ['exercise_name', 'sets', 'reps', 'rest_seconds', 'superset_group', 'notes'],
  ...
  required: ['routine_name', 'routine_description', 'medical_rationale', 'days'],
  additionalProperties: false
}
...
1. AGRUPACIÓN DE GRUPOS MUSCULARES POR DÍA (split sinergista — OBLIGATORIO salvo override en notas):${allowChest ? `
   ...
   IMPORTANTE — "FRECUENCIA 2" NO ES SINÓNIMO DE CAMBIAR A UPPER/LOWER: si el admin pide frecuencia 2 (en un grupo concreto o "en todos"), NO abandones el split sinérgico de arriba para montar un Upper/Lower/Push-Pull-Legs genérico — ese tipo de split diluye el volumen por sesión y solo tiene sentido para principiantes o cuando el admin lo pide EXPLÍCITAMENTE por su nombre ("quiero upper/lower", "hazlo full body"). La forma correcta de dar frecuencia 2 DENTRO del split sinérgico es la técnica de bi-serie de la REGLA 10: el grupo que necesita el segundo estímulo se añade en bi-serie al final de otro día afín, sin tocar la estructura, los nombres de los días ni el volumen del resto.
...
10. FRECUENCIA 2 EN UN GRUPO CONCRETO — SIEMPRE VÍA BI-SERIE, NUNCA CAMBIANDO EL SPLIT: aplica esta técnica cada vez que un grupo o subgrupo necesite un segundo estímulo semanal, tanto si lo detectas tú (el bloque de HISTORIAL Y CONTEXTO indica que está visiblemente menos desarrollado que el resto, ej. "hombro lateral rezagado", "gemelo poco desarrollado") como si el admin lo pide explícitamente ("dale frecuencia 2 a X", "que toque pierna dos veces"). NO le añadas un día entero ni reestructures el split (ver aviso de la regla 1): dale ese segundo estímulo metiéndolo en BI-SERIE al FINAL del último ejercicio de un día donde ya se entrene algo sinérgico o cercano a ese subgrupo. Ejemplos concretos:
   - Deltoide LATERAL rezagado → bi-serie de elevaciones laterales al cierre del día de pecho (o del día de empuje/hombro).
   - Deltoide POSTERIOR rezagado → bi-serie de aperturas posteriores / pájaros / face pull al cierre del día de espalda.
   - Deltoide FRONTAL rezagado → bi-serie de elevaciones frontales al cierre del día de pecho.
   - Cualquier otro grupo rezagado (gemelo, glúteo, etc.) → mismo criterio: bi-serie de un ejercicio de aislamiento de ese grupo al final de un día afín, DISTINTO del día donde ya tiene su volumen principal, para que sume una segunda sesión semanal (frecuencia 2) sin desequilibrar el resto de la rutina.
   Indica el superset_group de esa bi-serie igual que en la regla 7.
...
7. Si el plan menciona bi-series / tri-series, asigna superset_group entero (1, 2, 3...) a los ejercicios agrupados (mismo número = mismo grupo). Usa 0 en ejercicios individuales.

FORMATO JSON DE RESPUESTA (campos "medical_rationale" y "notes": deja el string vacío "" si no aplica, nunca los omitas):
{
  "routine_name": "string",
  "routine_description": "string",
  "medical_rationale": "string (vacío si no aplica)",
  "days": [
    {
      "day_number": 1,
      ...
      "exercises": [
        {
          ...
          "rest_seconds": 90,
          "superset_group": 0,
          "notes": "string (vacío si no aplica)"
        }
      ]
    }
  ]
...
// Indexa el catálogo por nombre canónico Y por cada alias conocido
// (columna name_aliases), para que un nombre que un entrenador o la IA
// escriban de forma distinta pero equivalente ("Sentadilla Hack" en vez de
// "Hack squat") resuelva al ejercicio real en vez de caer al fuzzy-match o,
// peor, a una sustitución aleatoria del mismo grupo muscular.
function buildCatalogByName(catalog) {
  const map = new Map()
  for (const e of catalog) {
    map.set(normalize(e.name), e)
    for (const alias of e.name_aliases || []) {
...
export async function gatherMemberTrainingContext(supabase, member_id) {
  ...
  try {
    ...
    // Última nota registrada de cada ejercicio (independiente de si tiene
    // series: el socio puede dejar una nota sin rellenar peso/reps).
    const lastNoteByExercise = new Map()
    for (const row of noteRows || []) {
      const key = (row.exercise_name || '').trim()
      if (!key || lastNoteByExercise.has(key)) continue // ya tiene la más reciente
      lastNoteByExercise.set(key, row.note)
    }
    ...
    if (Array.isArray(logs) && logs.length > 0) {
      ...
      for (const [name, entry] of byExercise) {
        const sets = entry.sets
          ...
          .join(', ')
        const note = lastNoteByExercise.get(name)
        lastNoteByExercise.delete(name) // ya incluida en esta línea
        lines.push(note ? `${name}: ${sets} (nota del socio: "${note}")` : `${name}: ${sets}`)
        if (lines.length >= 15) break
      }
      // Ejercicios con nota pero sin ninguna serie registrada en el rango.
      for (const [name, note] of lastNoteByExercise) {
        if (lines.length >= 15) break
        lines.push(`${name}: sin series registradas (nota del socio: "${note}")`)
      }
      if (lines.length > 0) {
        ctx.setLogs = lines.join(' | ')
      }
    } else if (lastNoteByExercise.size > 0) {
      ctx.setLogs = [...lastNoteByExercise.entries()]
        .slice(0, 15)
        .map(([name, note]) => `${name}: sin series registradas (nota del socio: "${note}")`)
        .join(' | ')
    }
  } catch (e) {
...
export async function generateRoutineForMember({ supabase, member_id = null, trainer_id = null, criteria }) {
  ...
  // Si sex es null (ficha incompleta — 8 socios reales lo tienen así hoy),
  // NO lo tratamos como "mujer/otro": eso excluía pecho por completo sin que
  // nadie lo hubiera decidido. Ante dato ausente, mejor incluir pecho (como
  // 'male') que dejar un grupo muscular entero sin ningún ejercicio — el
  // coste de incluir un press de más es mucho menor que el de omitir todo
  // un grupo. 'female'/'other' siguen respetándose tal cual: es una decisión
  // deliberada del gimnasio (no se les asigna pecho por defecto), salvo que
  // el admin lo pida explícitamente en las notas ("quiere trabajar pecho").
  const chestRequestedExplicitly = /pecho/i.test(notes || '')
  const allowChest = memberSex === 'male' || !memberSex || chestRequestedExplicitly

  const { data: catalog, error: catalogError } = await supabase
    .from('exercises')
    .select('id, name, name_aliases, muscle_primary, muscle_secondary, equipment, difficulty, default_sets, default_reps, default_rest_seconds, only_male')
    .eq('is_global', true)
  ...
  if (trainingContext.setLogs) {
    historyParts.push(`SERIES REALES REGISTRADAS POR EL SOCIO (última sesión de cada ejercicio, últimas 6 semanas — es la carga que mueve DE VERDAD, más fiable que un récord puntual):\n${trainingContext.setLogs}\nÚsalo para progresar de forma realista: si viene cumpliendo el rango alto de repeticiones con un peso, plantea subir carga; si se queda corto, mantén o baja. No propongas cargas desconectadas de estos números. Cuando un ejercicio venga con "nota del socio", tenla en cuenta de verdad: si dice que le costó mucho más de lo normal, no subas la carga a lo loco ese ejercicio; si dice que tuvo que hacerlo con otra máquina/variante porque la habitual no estaba disponible o no le iba bien, considera proponer directamente esa alternativa o una equivalente en vez de insistir en el ejercicio original.`)
  }
  ...
  if (physiqueAnalysis) {
    historyParts.push(`ANÁLISIS VISUAL DE SUS FOTOS DE PROGRESO MÁS RECIENTES (esto es lo que se VE — si detectas aquí un grupo o subgrupo concreto visiblemente rezagado, aplica la REGLA 10 del prompt: dale frecuencia 2 metiéndolo en bi-serie al final de un día afín, no le añadas un día entero. Si algo ya se ve muy trabajado, no sobrecargues más ese grupo):\n${physiqueAnalysis}`)
  }
...
Esta rutina aplica a una persona con condición especial (${conditionSummary}). Rellena el campo "medical_rationale" (string, máximo 400 caracteres) explicando en español, en tono cercano y en SEGUNDA PERSONA (de tú), por qué esta rutina es beneficiosa para esa condición.
...
Si NO aplica condición especial, deja "medical_rationale" como string vacío "".`
    : ''
...
Diseña exactamente ${days_per_week} días siguiendo el FORMATO DE TU RESPUESTA del system prompt (texto plano, NO JSON).${rationaleInstruction}`
  ...
  const planningResponse = await anthropic.messages.create({
    model: CLAUDE_MODEL,
    max_tokens: 3500,
    system: buildPlanningPrompt(allowChest),
    thinking: NO_THINKING,
    output_config: MEDIUM_EFFORT,
    messages: [{ role: 'user', content: planningCriteria }],
  })
  if (planningResponse.stop_reason === 'refusal') {
    const err = new Error('No se pudo planificar la rutina (paso 1). Inténtalo de nuevo.')
    err.status = 500
    throw err
  }
  const planText = extractText(planningResponse) || ''
  if (!planText.trim()) {
    const err = new Error('No se pudo planificar la rutina (paso 1). Inténtalo de nuevo.')
    err.status = 500
    throw err
...
Convierte el plan anterior en JSON siguiendo el FORMATO JSON DE RESPUESTA del system prompt. Conserva sets/reps/descansos del plan literalmente. ${rationaleInstruction ? '\n\n' + rationaleInstruction : ''}`
  ...
  const response = await anthropic.messages.create({
    ...
    thinking: NO_THINKING,
    output_config: { ...MEDIUM_EFFORT, format: { type: 'json_schema', schema: ROUTINE_SCHEMA } },
    messages: [{ role: 'user', content: formattingMessage }],
  })

  let routineJson
  try {
    if (response.stop_reason === 'refusal') throw new Error('refusal')
    routineJson = JSON.parse(extractText(response))
  } catch {
  ...
  const catalogByName = buildCatalogByName(filteredCatalog)
...
export async function refineRoutineDraft({ supabase, member_id = null, currentRoutine, correction }) {
  ...
  if (member_id) {
    ...
    // Mismo criterio que generateRoutineForMember: sex ausente NO es
    // "mujer/otro", y una corrección que pida pecho explícitamente ("añade
    // press de pecho", "quiere trabajar pecho") lo habilita igualmente.
    const chestRequestedExplicitly = /pecho/i.test(correction || '')
    allowChest = memberProfile?.sex === 'male' || !memberProfile?.sex || chestRequestedExplicitly
  }
...
"${correction}"
${memoryBlock ? `\n${memoryBlock}\n` : ''}${physiqueAnalysis ? `\nANÁLISIS VISUAL DE SUS FOTOS DE PROGRESO MÁS RECIENTES (ten esto en cuenta si la corrección tiene que ver con énfasis/volumen de algún grupo muscular). Si hay un grupo o subgrupo visiblemente rezagado (ej. deltoide lateral/posterior/frontal, gemelo...) y la corrección pide darle más presencia, la forma correcta es meterlo en BI-SERIE al final del último ejercicio de un día afín donde aún no tenga su volumen principal — así consigue frecuencia 2 esa semana sin añadirle un día entero:\n${physiqueAnalysis}\n` : ''}
...
4. Devuelve el JSON completo de la rutina corregida (routine_name, routine_description, medical_rationale, days[].day_number/day_name/exercises[].exercise_name/sets/reps/rest_seconds/superset_group/notes). Conserva "medical_rationale" tal cual estaba en la rutina actual (string vacío si no tenía); "notes" también como string vacío "" si no aplica, nunca null.`
