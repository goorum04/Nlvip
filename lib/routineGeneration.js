...
export async function generateRoutineForMember({ supabase, member_id = null, trainer_id = null, criteria }) {
  ...
  // un grupo. 'female'/'other' siguen respetándose tal cual: son una
  // elección explícita del socio, no un dato que falte.
  const allowChest = memberSex === 'male' || !memberSex
...
export async function refineRoutineDraft({ supabase, member_id = null, currentRoutine, correction }) {
  ...
  if (member_id) {
    ...
    // Mismo criterio que generateRoutineForMember: sex ausente NO es "mujer/otro".
    allowChest = memberProfile?.sex === 'male' || !memberProfile?.sex
  }
