...
const options = {
  ...
  production: true, // TestFlight and App Store require production APNs environment
}
...
/**
 * Server-side status snapshot, safe to expose (no secret values).
 */
export function getApnStatus() {
  return {
    providerReady: !!apnProvider,
    initError: apnInitError,
    keyIdSet: !!process.env.APNS_KEY_ID,
    teamIdSet: !!process.env.APNS_TEAM_ID,
    p8KeySet: !!process.env.APNS_P8_KEY,
    p8KeyBytes: process.env.APNS_P8_KEY?.length || 0,
    bundleId: process.env.APNS_BUNDLE_ID || 'com.leonardos.app',
