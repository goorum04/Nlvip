...
`)
  ...
  async generate_member_routine(params) {
    ...
    let result
    try {
      result = await generateRoutineForMember({
        supabase: adminClient,
        member_id,
        criteria: {
          days_per_week,
      ...
      // DIAGNÓSTICO TEMPORAL: exponer el stack en el mensaje de error porque
      // no hay acceso a los runtime logs de Vercel en este momento. Quitar
      // en cuanto se identifique la causa real de "a is not iterable".
      e.message = `DIAG[generate_member_routine]: ${e.message}\n${e.stack || ''}`
      throw e
    }
