...
`)
  ...
  async generate_ai_diet_from_recipes(params) {
    ...
    if (!baseUrl) {
      throw new Error('No se pudo determinar la URL base de la app (define NEXT_PUBLIC_APP_URL o NEXT_PUBLIC_SITE_URL en el entorno).')
    }
    ...
    async save_ai_diet(params) {
      ...
      for (const key of mealKeys) {
        ...
        savedRecipes.push({ id: recipe.id, meal_slot: key })
      }
