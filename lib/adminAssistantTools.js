...
export const TOOLS_DEFINITIONS = [
  ...
  },
  {
    type: "function",
    function: {
      name: "set_routine_dropset",
      description: "Marcar (o desmarcar) un ejercicio del preview como DROPSET. Úsalo cuando el admin diga 'ponle un dropset al press banca', 'haz dropset las curls del día 4 con 2 drops', 'quita el dropset del press', 'añade dropset de 3 al jalón'. Un dropset = tras cada serie principal, el atleta reduce el peso al fallo N veces sin descanso entre ellas. Devuelve la rutina actualizada (NO la guarda).",
      parameters: {
        type: "object",
        properties: {
          routine_data: { type: "object" },
          day_index: { type: "integer", description: "Día (1-based) donde está el ejercicio" },
          exercise_name: { type: "string", description: "Nombre del ejercicio (búsqueda case-insensitive parcial)" },
          drops: { type: "integer", description: "Número de drops por serie. 0 = quitar dropset. 1-3 = añadir dropset con esa cantidad de drops. Por defecto 2 si el admin no lo especifica." }
        },
        required: ["routine_data", "day_index", "exercise_name"]
      }
    }
...
`)
  ...
  // 💥 Marcar/desmarcar un ejercicio como dropset
  async set_routine_dropset(params) {
    const { routine_data, day_index, exercise_name, drops = 2 } = params
    const updated = JSON.parse(JSON.stringify(routine_data))
    const day = _getDay(updated, day_index)
    const idx = _findExerciseIdx(day, exercise_name)
    const ex = day.exercises[idx]
    const dropsNum = Number.isInteger(drops) ? drops : 2
    if (dropsNum <= 0) {
      ex.dropset = 0
      return {
        success: true,
        ...
        member_name: routine_data.member_name || null,
        member_id: routine_data.member_id || null,
        message: `✅ Dropset eliminado de "${ex.exercise_name}" (día ${day_index}).`
      }
    }
    if (dropsNum > 5) {
      throw new Error('Máximo 5 drops por serie. Más de eso no aporta valor y agrava la fatiga.')
    }
    ex.dropset = dropsNum
    return {
      success: true,
      ...
      member_name: routine_data.member_name || null,
      member_id: routine_data.member_id || null,
      message: `✅ "${ex.exercise_name}" (día ${day_index}) ahora con dropset ×${dropsNum}: tras cada serie principal, ${dropsNum} reducción(es) de peso al fallo sin descanso.`
    }
  },
  ...
  return toolCalls.map(call => {
    ...
    switch (call.function.name) {
      ...
      case 'modify_routine_day':
        ...
        break
      case 'set_routine_dropset':
        icon = '💥'
        description = (args.drops === 0)
          ? `Quitar dropset de "${args.exercise_name}" (día ${args.day_index})`
          : `Marcar "${args.exercise_name}" como dropset ×${args.drops ?? 2} (día ${args.day_index})`
        break
      default:
