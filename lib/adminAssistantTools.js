...
export const toolExecutors = {
  async search_exercise_catalog(params) {
    ...
    let q = client
      .from('exercises')
      .select('name, muscle_primary, default_sets, default_reps, default_rest_seconds')
      .eq('is_global', true)
      .order('name')
      .limit(10)
    ...
    return {
      ...
      exercises: data || [],
      count: data?.length || 0,
      message: data?.length ? `Encontré ${data.length} ejercicio(s)` : 'Ningún ejercicio del catálogo coincide con esa búsqueda — prueba con otro término o grupo muscular.'
    }
  },
