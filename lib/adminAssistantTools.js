...
`)
  ...
  async generate_ai_diet_from_recipes(params) {
    const { member_id, member_name, prompt, goal = 'maintain', weight_kg = 75, exclude_ingredients = '', preferences = '' } = params
    const client = getSupabaseClient(params)
...
export async function executeTool(toolName, params, adminToken) {
  ...
  // Si hay token de admin, crear un cliente autenticado para las RPCs
  ...
  if (adminToken) {
    ...
    // Simular sesión autenticada sin conocer el user ID (el RPC lo sacará de auth.uid())
    await authClient.auth.setSession({
      access_token: adminToken,
      refresh_token: '',
      user: { id: '', email: '' } // El RPC usará auth.uid() para obtener el ID real
    })
