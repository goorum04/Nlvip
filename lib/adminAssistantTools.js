...
export const TOOLS_DEFINITIONS = [
  ...
  },
  {
    ...
    function: {
      ...
      description: "Asignar una rutina de entrenamiento a un socio. Primero lista las rutinas disponibles si no se especifica. Un socio puede tener a la vez una rutina 'principal' y otra 'alternativa' (p.ej. para días sin tiempo o vacaciones); por defecto se asigna como principal.",
      parameters: {
        type: "object",
        properties: {
          member_id: {
          ...
          routine_slot: {
            ...
            description: "Qué rutina del socio sustituye: 'principal' (por defecto) o 'alternativa' (días sin tiempo, vacaciones, etc.)"
          }
        },
  ...
  },
  {
    ...
    function: {
      ...
      description: "Generar una dieta COMPLETA nueva, desde cero, con alimentos reales para un socio usando el SISTEMA NL ELITE. Produce comidas detalladas con 4 opciones intercambiables por toma (desayuno, comida, merienda, cena) con ingredientes y pesos exactos en gramos. Lee automáticamente el formulario de onboarding del socio (restricciones, preferencias, horario, objetivo). Úsalo cuando el admin pida 'genera una dieta', 'hazle una dieta', 'crea un plan nutricional' o similar. IMPORTANTE: para AJUSTAR una dieta que ya le enseñaste (bájale las calorías, quita un alimento, cambia una opción...) NO uses esta herramienta — usa refine_ai_diet, que edita la dieta real en vez de inventar una completamente nueva. Devuelve un preview; usa save_ai_diet para guardarla y asignarla.",
      parameters: {
        type: "object",
        properties: {
          member_id: {
          ...
          target_calories: {
            type: "number",
            description: "IMPORTANTE: si el admin da una cifra de calorías concreta o aproximada (ej. 'ponle unas 2800 kcal', 'bájale a 2500', 'súbele 200 kcal' sobre una dieta previa de la que conoces el total), calcula el número resultante y pásalo aquí. Sin este campo, las calorías SIEMPRE se recalculan con la fórmula estándar del objetivo (TDEE ±%) y CUALQUIER cifra concreta que haya pedido el admin se ignora — no lo dejes en manos del cálculo estándar cuando el admin ya dio un número."
          }
        },
  ...
  },
  {
    ...
    function: {
      name: "refine_ai_diet",
      description: "Aplicar un AJUSTE PUNTUAL sobre una dieta que ya generaste/enseñaste (quitar/cambiar un alimento, subir o bajar calorías, sustituir una opción, etc.), conservando el resto EXACTAMENTE igual. Úsala en vez de generate_ai_diet_from_recipes para cualquier corrección — inventar una dieta nueva de cero cuando solo se pide un cambio puntual hace que cambien alimentos y comidas que nadie pidió tocar. Necesitas el contenido EXACTO de la dieta actual: solo lo tienes si generate_ai_diet_from_recipes (o una corrección anterior) te lo devolvió DENTRO de este mismo turno, o si el mensaje del admin viene precedido de un bloque [CONTEXTO INTERNO DEL SISTEMA] con la última dieta de ese socio — en ese caso pasa ese contenido tal cual en current_diet_content. Si no tienes ninguno de los dos, NO inventes el contenido: usa generate_ai_diet_from_recipes de nuevo con el prompt describiendo la dieta completa que quieres.",
      parameters: {
        type: "object",
        properties: {
          member_id: {
          ...
          current_diet_content: {
            type: "string",
            description: "Contenido EXACTO (texto completo) de la dieta actual, tal cual la devolvió generate_ai_diet_from_recipes/refine_ai_diet o el bloque de contexto interno. NO lo resumas ni lo reescribas."
          },
          current_calories: { type: "number", description: "Calorías totales actuales de la dieta" },
          current_protein_g: { type: "number", description: "Proteína actual en gramos" },
          current_carbs_g: { type: "number", description: "Carbohidratos actuales en gramos" },
          current_fat_g: { type: "number", description: "Grasa actual en gramos" },
          correction: {
            type: "string",
            description: "Instrucción del admin en lenguaje natural describiendo el ajuste (ej. 'quita el salmón de la cena, no le gusta', 'bájale unas 200 kcal', 'cambia la opción 2 del desayuno por algo con avena')"
          }
        },
        required: ["member_id", "current_diet_content", "correction"]
      }
    }
  },
  {
  ...
  },
  {
    ...
    function: {
      ...
      description: "Guardar la rutina generada por IA como plantilla y ASIGNARLA al socio. Usa esto SOLO cuando el admin confirme con 'asígnala', 'guárdala', 'confirmar', 'dale', etc. Un socio puede tener a la vez una rutina 'principal' y otra 'alternativa' (días sin tiempo, vacaciones); por defecto se asigna como principal.",
      parameters: {
        type: "object",
        properties: {
          member_id: {
          ...
          routine_slot: {
            type: "string",
            enum: ["principal", "alternativa"],
            description: "Qué rutina del socio sustituye: 'principal' (por defecto) o 'alternativa'"
          }
        },
  ...
  },
  {
    ...
    function: {
      ...
      description: "Buscar ejercicios reales del catálogo oficial del gimnasio por nombre parcial o grupo muscular. ÚSALA SIEMPRE antes de swap_routine_exercise / add_routine_exercise cuando no estés 100% seguro del nombre EXACTO tal cual está en el catálogo — un nombre que 'suena bien' pero no coincide literalmente (ej. 'Curl predicador' en vez de 'Curl en banco predicador') hace que esas herramientas fallen. Devuelve hasta 10 coincidencias con su nombre exacto.",
      parameters: {
        type: "object",
        properties: {
          query: { type: "string", description: "Término de búsqueda: nombre o parte del nombre del ejercicio (ej: 'curl', 'press banca')" },
          muscle_group: { type: "string", description: "Grupo muscular para filtrar/explorar (ej: 'bíceps', 'pecho', 'espalda'). Opcional, se puede combinar con query o usar solo." }
        }
      }
    }
  },
  ...
  },
  {
    ...
    function: {
      ...
      description: "Guardar un dato duradero sobre UN socio en concreto que NO sea una aversión de alimento (para eso usa add_food_aversion). Se ACUMULA en memoria y se usará automáticamente cada vez que se le genere una rutina o dieta a ese socio, sin que el admin tenga que repetirlo. Úsalo cuando el admin diga algo permanente sobre un socio: lesiones o molestias ('hombro sensible al hacer press'), preferencias de ejercicios ('prefiere barra recta en curl'), o cualquier otra cosa que deba tenerse en cuenta siempre a partir de ahora con ese socio.",
      parameters: {
        type: "object",
        properties: {
          member_id: {
          ...
          note: {
            type: "string",
            description: "La nota, breve y concreta (ej: 'Hombro sensible al hacer press, evitar press militar con barra')"
          }
        },
  ...
  },
  {
    ...
    function: {
      ...
      description: "Ver todas las notas duraderas guardadas de un socio (memoria del asistente sobre ese socio, aparte de sus aversiones de alimentos).",
      parameters: {
        type: "object",
        properties: {
          member_id: {
  ...
  },
  {
    type: "function",
    function: {
      name: "remove_member_note",
      description: "Eliminar UNA nota específica de un socio (si el admin se equivocó o ya no aplica). Busca por coincidencia parcial del texto.",
      parameters: {
        type: "object",
        properties: {
          member_id: {
          ...
          note_text: {
            type: "string",
            description: "Texto (parcial o completo) de la nota a eliminar"
          }
        },
  ...
  },
  {
    type: "function",
    function: {
      name: "remember_admin_preference",
      description: "Guardar una preferencia GENERAL de cómo el admin quiere que se trabaje, NO ligada a un socio concreto (para eso usa add_member_note). Se aplicará automáticamente en TODAS las futuras generaciones de rutinas y dietas de CUALQUIER socio. Úsalo cuando el admin exprese una forma de trabajar recurrente: agrupaciones musculares que prefiere, terminología de ejercicios que usa siempre (ej: 'barra recta' para bíceps y tríceps), formato de resúmenes, o cualquier patrón que quiera que se respete siempre, o cuando diga explícitamente 'recuerda que...' / 'acuérdate de que...'.",
      parameters: {
        type: "object",
        properties: {
          note: {
            type: "string",
            description: "La preferencia, breve y concreta (ej: 'Para el día de empuje, agrupar hombro con tríceps aunque el socio sea hombre')"
          }
        },
        required: ["note"]
      }
  ...
  },
  {
    type: "function",
    function: {
      name: "list_admin_preferences",
      description: "Ver todas las preferencias generales de trabajo guardadas (memoria del asistente sobre cómo le gusta trabajar al admin, aplicable a todos los socios).",
      parameters: {
        type: "object",
        properties: {}
      }
  ...
  },
  {
    type: "function",
    function: {
      name: "remove_admin_preference",
      description: "Eliminar UNA preferencia general específica (si el admin se equivocó o ya no aplica). Busca por coincidencia parcial del texto.",
      parameters: {
        type: "object",
        properties: {
          note_text: {
            type: "string",
            description: "Texto (parcial o completo) de la preferencia a eliminar"
          }
        },
        required: ["note_text"]
      }
...
// Dispara la generación del plan de recetas justo al asignar una dieta,
// en segundo plano (no bloquea la respuesta de la herramienta al admin).
function triggerRecipePlanGeneration(memberId, dietId, adminToken) {
  const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || process.env.NEXT_PUBLIC_SITE_URL
  if (!baseUrl || !memberId || !dietId || !adminToken) return
  fetch(`${baseUrl}/api/generate-recipe-plan`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${adminToken}` },
    body: JSON.stringify({ memberId, dietId, trainerId: null })
  }).catch(e => console.warn('Recipe plan generation failed:', e.message))
}

// Ejecutores de cada herramienta
export const toolExecutors = {
  async search_exercise_catalog(params) {
    const { query, muscle_group } = params
    const client = getSupabaseClient(params)
    let q = client
      .from('exercises')
      .select('name, name_aliases, muscle_primary, default_sets, default_reps, default_rest_seconds')
      .eq('is_global', true)
      .order('name')
    if (muscle_group) q = q.ilike('muscle_primary', `%${muscle_group}%`)
    const { data, error } = await q
    if (error) throw new Error(`Error consultando catálogo: ${error.message}`)

    // El nombre "oficial" del catálogo no siempre coincide con cómo lo pide
    // el admin (p.ej. "sentadilla hack" en vez de "hack squat"), así que
    // también se compara contra los alias conocidos (name_aliases), no solo
    // contra el nombre exacto.
    const needle = (query || '').trim().toLowerCase()
    const matches = needle
      ? (data || []).filter(e =>
          e.name.toLowerCase().includes(needle) ||
          (e.name_aliases || []).some(a => a.toLowerCase().includes(needle))
        )
      : (data || [])
    const results = matches.slice(0, 10).map(({ name, muscle_primary, default_sets, default_reps, default_rest_seconds }) =>
      ({ name, muscle_primary, default_sets, default_reps, default_rest_seconds }))

    return {
      success: true,
      exercises: results,
      count: results.length,
      message: results.length ? `Encontré ${results.length} ejercicio(s)` : 'Ningún ejercicio del catálogo coincide con esa búsqueda — prueba con otro término o grupo muscular.'
    }
  },
  ...
  async apply_full_member_plan(params) {
    ...
    // La dieta se asigna dentro del RPC; generamos el plan de recetas
    // acorde a sus macros en el mismo momento, igual que en MemberDetailPanel.
    if (data?.diet_id) {
      triggerRecipePlanGeneration(member_id, data.diet_id, params._adminToken)
    }
...
`)
  ...
  async assign_workout_to_member(params) {
    const { member_id, workout_id, routine_slot = 'principal' } = params
    const client = getSupabaseClient(params)
    ...
    if (error) throw new Error(error.message)
    return { success: true, message: routine_slot === 'alternativa' ? 'Rutina alternativa asignada correctamente' : 'Rutina asignada correctamente' }
  },
  ...
  async update_member_macros(params) {
    ...
    const client = getSupabaseClient(params)
    if (!params._callerId) throw new Error('No se pudo identificar al admin que hace el cambio (assigned_by requerido)')
    // Upsert into macro_goals (one row per member)
    const { error } = await client
      .from('macro_goals')
      .upsert(
        { member_id, assigned_by: params._callerId, calories, protein_g, carbs_g, fat_g, updated_at: new Date().toISOString() },
  ...
  async save_custom_recipe(params) {
    ...
    // Obtener el admin real (excluye cuentas de prueba @nlvipnutrition.internal;
    // ver checkin/submit/route.js para el bug real que motivó este filtro)
    const { data: admin } = await supabase.from('profiles').select('id').eq('role', 'admin').not('email', 'like', '%@nlvipnutrition.internal').limit(1).single();
  ...
  async generate_ai_diet_from_recipes(params) {
    const { member_id, prompt: extraNotes = '', goal = 'maintain', exclude_ingredients = '', preferences = '', target_calories = null } = params
    const client = getSupabaseClient(params)
    ...
    const result = await generateNLEliteDiet({
      ...
      preferences,
      extraNotes,
      calorieOverride: target_calories,
    })
  ...
  // 🩹 Ajuste puntual sobre una dieta ya generada (swap/quitar/subir·bajar
  // calorías...) usando refineDietDraft — edita el contenido real en vez de
  // regenerar una dieta nueva de cero como hacía antes generate_ai_diet_from_recipes
  // para cualquier corrección (eso cambiaba TODAS las comidas aunque solo se
  // pidiera tocar una).
  async refine_ai_diet(params) {
    const { member_id, current_diet_content, current_calories, current_protein_g, current_carbs_g, current_fat_g, correction } = params
    if (!current_diet_content || typeof current_diet_content !== 'string' || !current_diet_content.trim()) {
      throw new Error('No tengo el contenido exacto de la dieta actual para ajustarla. Esto pasa cuando la dieta se generó en un mensaje anterior (no en este mismo turno) y no hay contexto interno con ella: no debo inventar su contenido de memoria. Pídeme el cambio completo (ej. "genera de nuevo la misma dieta de [socio] pero sin X") y la regenero con el ajuste incluido.')
    }
    ...
    const { refineDietDraft } = await import('./dietGeneration')
    const result = await refineDietDraft({
      currentContent: current_diet_content,
      currentMacros: {
    ...
    const dietData = {
      fullDietContent: result.content,
      macros: result.macros,
      member_name: profile?.name || null,
      physique_analysis: result.physique_analysis || null,
    }
    ...
    return {
      ...
      diet_data: dietData,
      member_id,
      message: `✅ Dieta ajustada para ${dietData.member_name || 'el socio'}.${result.changeSummary ? '\n' + result.changeSummary : ''}`,
    }
  },
    ...
    async save_ai_diet(params) {
      ...
      const { error: mError } = await adminClient.from('macro_goals').upsert({
        member_id,
        assigned_by: params._callerId,
        calories: template.calories,
        protein_g: template.protein_g,
        carbs_g: template.carbs_g,
        fat_g: template.fat_g,
      }, { onConflict: 'member_id' })
      if (mError) console.warn('refine_ai_diet: no se pudieron actualizar macro_goals:', mError.message)

      // 4. Generar el plan de recetas acorde a las macros, en el mismo momento
      // de asignar la dieta (igual que en MemberDetailPanel).
      triggerRecipePlanGeneration(member_id, template.id, params._adminToken)
  ...
  async generate_member_routine(params) {
    ...
    return {
      ...
      physique_analysis: result.physique_analysis,
  ...
  async save_member_routine(params) {
    const { member_id, trainer_id = null, routine_data: rawRoutineData, routine_slot = 'principal' } = params
    // El modelo a veces pasa el resultado completo de generate_member_routine
    // en lugar de solo el campo routine_data. Manejamos ambos casos.
    const routine_data = rawRoutineData?.routine_data || rawRoutineData
    if (!routine_data || !Array.isArray(routine_data.days) || routine_data.days.length === 0) {
      throw new Error('No hay datos de rutina para guardar. Asegúrate de pasar routine_data correcto.')
    }
    ...
    return {
      success: true,
      message: `✅ Rutina "${template.name}" guardada y asignada al socio${routine_slot === 'alternativa' ? ' como rutina alternativa' : ''}.`,
      workout_template_id: template.id
    }
  },
  ...
  async swap_routine_exercise(params) {
    ...
    if (!routine_data || !Array.isArray(routine_data.days)) {
      throw new Error('No tengo el JSON exacto de la rutina para editarla. Esto pasa cuando la rutina se generó en un mensaje anterior (no en este mismo turno): no debo inventar su contenido de memoria. Pídeme el cambio completo (ej. "genera de nuevo la misma rutina de [socio] pero con X") y la regenero con el ajuste incluido.')
    }
  ...
  // 🧠 Memoria del asistente: notas duraderas por socio
  async add_member_note(params) {
    const { member_id, note } = params
    const client = getSupabaseClient(params)
    const { data, error } = await client.rpc('rpc_add_member_note', {
      p_member_id: member_id,
      p_note: note
    })
    if (error) throw new Error(error.message)
    return { success: true, message: data.message, total_notes: data.total_notes }
  },

  async list_member_notes(params) {
    const { member_id } = params
    const client = getSupabaseClient(params)
    const { data, error } = await client.rpc('rpc_list_member_notes', { p_member_id: member_id })
    if (error) throw new Error(error.message)
    if (!Array.isArray(data) || data.length === 0) {
      return { success: true, notes: [], message: 'Este socio no tiene notas guardadas.' }
    }
    return {
      success: true,
      notes: data,
      count: data.length,
      message: `El socio tiene ${data.length} nota(s) guardada(s):`,
      note_list: data.map(n => `• ${n.note}`).join('\n')
    }
  },

  async remove_member_note(params) {
    const { member_id, note_text } = params
    const client = getSupabaseClient(params)
    const { data, error } = await client.rpc('rpc_remove_member_note', {
      p_member_id: member_id,
  ...
  // 🧠 Memoria del asistente: preferencias generales del admin (no ligadas a un socio)
  async remember_admin_preference(params) {
    const { note } = params
    const client = getSupabaseClient(params)
    const { data, error } = await client.rpc('rpc_add_admin_preference', { p_note: note })
    if (error) throw new Error(error.message)
    return { success: true, message: data.message, total_preferences: data.total_preferences }
  },

  async list_admin_preferences(params) {
    const client = getSupabaseClient(params)
    const { data, error } = await client.rpc('rpc_list_admin_preferences')
    if (error) throw new Error(error.message)
    if (!Array.isArray(data) || data.length === 0) {
      return { success: true, preferences: [], message: 'No hay preferencias generales guardadas todavía.' }
    }
    return {
      success: true,
      preferences: data,
      count: data.length,
      message: `Hay ${data.length} preferencia(s) general(es) guardada(s):`,
      preference_list: data.map(p => `• ${p.note}`).join('\n')
    }
  },

  async remove_admin_preference(params) {
    const { note_text } = params
    const client = getSupabaseClient(params)
    const { data, error } = await client.rpc('rpc_remove_admin_preference', { p_note_text: note_text })
    if (error) throw new Error(error.message)
    return { success: true, message: data.message, total_preferences: data.total_preferences }
  }
  }
...
async function _findCatalogExercise(name) {
  ...
  const { data, error } = await adminClient
    .from('exercises')
    .select('id, name, name_aliases, muscle_primary, default_sets, default_reps, default_rest_seconds')
    .eq('is_global', true)
    .order('name')
  if (error) throw new Error(`Error consultando catálogo: ${error.message}`)

  // Igual que search_exercise_catalog: el admin/la IA pueden pedir un
  // ejercicio por un nombre distinto al oficial del catálogo (p.ej.
  // "sentadilla hack" en vez de "hack squat"), así que también se compara
  // contra los alias conocidos, no solo contra el nombre exacto.
...
export async function executeTool(toolName, params, adminToken) {
  ...
  if (adminToken) {
    ...
    enhancedParams = { ...params, _supabaseClient: authClient, _callerId: caller?.id || null, _adminToken: adminToken }
  }
  ...
  return toolCalls.map(call => {
    ...
    switch (call.function.name) {
      ...
      case 'remove_food_aversion':
        ...
        break
      case 'add_member_note':
        icon = '🧠'
        description = `Recordar sobre el socio: "${args.note}"`
        break
      case 'list_member_notes':
        icon = '📋'
        description = `Ver notas guardadas del socio`
        break
      case 'remove_member_note':
        icon = '❌'
        description = `Eliminar nota del socio: "${args.note_text}"`
        break
      case 'remember_admin_preference':
        icon = '🧠'
        description = `Recordar preferencia general: "${args.note}"`
        break
      case 'list_admin_preferences':
        icon = '📋'
        description = `Ver preferencias generales guardadas`
        break
      case 'remove_admin_preference':
        icon = '❌'
        description = `Eliminar preferencia general: "${args.note_text}"`
        break
      default:
