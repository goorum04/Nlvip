...
import { DIET_TEMPLATE } from './dietTemplate.js'
...
export const TOOLS_DEFINITIONS = [
  ...
  },
  {
    ...
    function: {
      ...
      parameters: {
        type: "object",
        properties: {
          query: { type: "string", description: "Término de búsqueda (ej: 'pollo', 'pasta', 'cena saludable')" },
  ...
  },
  {
    ...
    function: {
      ...
      description: "Importa masivamente recetas de Spoonacular al catálogo local (ej: para 'keto', 'vegano', 'alto en proteína'). Esto pre-pobla el sistema con contenido premium.",
      parameters: {
        type: "object",
        properties: {
          queries: { type: "array", items: { type: "string" }, description: "Lista de términos de búsqueda (ej: ['keto', 'desayuno vegano', 'cena de pollo'])" },
...
export const toolExecutors = {
  ...
  async apply_full_member_plan({ member_id, goal, weight_kg }) {
    ...
    // Attempt auto-linking recipes to the new diet if possible
    try {
      const dietId = data?.diet_id
      if (dietId) {
        const mealCalories = Math.round((data.calories || 2000) / 3)
      ...
      console.error('Auto-linking recipes failed:', err)
    }
          ...
          for (const r of result.recipes) {
            ...
            if (openaiKey) {
              try {
                ...
                const resJson = typeof response.data.choices[0].message.content === 'string' 
