...
function triggerRecipePlanGeneration(memberId, dietId) {
  const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || process.env.NEXT_PUBLIC_SITE_URL
  if (!baseUrl || !memberId || !dietId) return
...
export const toolExecutors = {
  ...
  async apply_full_member_plan(params) {
    ...
    if (data?.diet_id) {
      triggerRecipePlanGeneration(member_id, data.diet_id)
    }
...
`)
  ...
  async refine_ai_diet(params) {
    ...
    async save_ai_diet(params) {
      ...
      triggerRecipePlanGeneration(member_id, template.id)
