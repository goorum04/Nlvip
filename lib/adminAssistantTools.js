...
`)
  ...
  async generate_ai_diet_from_recipes(params) {
    ...
    if (profile) {
      if (!weight_kg || weight_kg === 75) weight_kg = profile.weight_kg || 75
    ...
    // Llamar directamente a la función de librería (evita fetch interno a localhost
    // que falla en Vercel serverless porque no hay ningún servidor en puerto 3000).
    const { generateSpoonacularDietPlan } = await import('./spoonacularDiet')
    const dietData = await generateSpoonacularDietPlan({
      member_id,
      member_name,
      prompt,
      goal,
    ...
    async save_ai_diet(params) {
      ...
      // 6. Generar plan de recetas automáticamente
      const baseUrl = process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'
      fetch(`${baseUrl}/api/generate-recipe-plan`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ memberId: member_id, dietId: template.id })
