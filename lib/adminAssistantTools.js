...
`)
  ...
  async search_recipe_ideas({ query = '', max_calories, min_protein, max_carbs, diet, limit = 5 }) {
    try {
      let url = `https://api.spoonacular.com/recipes/complexSearch?apiKey=${process.env.SPOONACULAR_API_KEY}&addRecipeInformation=true&addRecipeNutrition=true&fillIngredients=true&number=${limit}`;
