...
export default function MemberDashboard({ user, profile, setProfile, onLogout }) {
  ...
  const handleAddProgress = async (e) => {
    ...
    try {
      ...
      // Notificar al trainer/admin de forma silenciosa (no bloquea al socio si falla)
      try {
        const { data: { session } } = await supabase.auth.getSession()
        await fetch('/api/progress/notify-trainer', {
          method: 'POST',
          headers: { 'Authorization': `Bearer ${session?.access_token}` },
        })
      } catch (e) {
        console.warn('[notify-trainer] Error al notificar:', e)
      }
    } catch (error) {
      toast({ title: 'Error', description: error.message, variant: 'destructive' })
    } finally {
