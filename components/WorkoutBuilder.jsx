...
// Selector de ejercicios desde el catálogo
function ExerciseCatalogPicker({ onSelect }) {
  ...
  const search = async () => {
    ...
    try {
      let q = supabase
        .from('exercises')
        .select('id, name, muscle_primary, equipment, difficulty, default_sets, default_reps, default_rest_seconds, description')
        .eq('is_global', true)
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button
          ...
          variant="outline"
          size="sm"
          className="border-violet-500/30 text-violet-400 hover:bg-violet-500/10 rounded-xl whitespace-nowrap px-3"
        >
        ...
        <div className="flex-1 space-y-3">
          <div className="flex gap-2">
            <Input
              ...
              className="flex-1 bg-black/50 border-violet-500/20 rounded-xl text-white"
            />
            <ExerciseCatalogPicker
              onSelect={(catalogEx) => {
                const updated = {
                  ...localExercise,
                  name: catalogEx.name,
                  sets: catalogEx.default_sets,
                  reps: catalogEx.default_reps,
                  rest_seconds: catalogEx.default_rest_seconds,
                  description: catalogEx.description || localExercise.description
                }
                setLocalExercise(updated)
                onUpdate(updated)
              }}
