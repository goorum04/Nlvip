  ...
  const handleRefineDraft = async (correctionText) => {
    ...
    try {
      ...
      setDietDraft(prev => ({
        ...
        rationale: result.explanation ? `${prev.rationale ? prev.rationale + '\n\n' : ''}• Último ajuste (${text}): ${result.explanation}` : prev.rationale
      }))
      setDraftCorrectionHistory(prev => [...prev, { userText: text, aiExplanation: result.explanation || result.changeSummary }])
      setDraftCorrection('')
                  ...
                  {(dietRequests || []).map(req => (
                    <div key={req.id} className="p-4 bg-black/60 rounded-2xl border border-violet-500/20 flex flex-col md:flex-row md:items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        >
                          {generatingDietId === req.id ? <Loader2 className="w-4 h-4 animate-spin mr-1" /> : <Bot className="w-4 h-4 mr-1" />}
                          Generar con IA
                        </Button>
                      </div>
                    </div>
