...
--
-- IDEMPOTENTE: No hace nada si la cuenta ya existe y está completa.
-- =========================================================================
...
BEGIN
  ...
  -- -----------------------------------------------------------------------
  -- Caso A: el usuario auth YA existe (de un intento previo o del trigger).
  --         Solo completar el perfil y sembrar actividad.
  -- -----------------------------------------------------------------------
  IF EXISTS (SELECT 1 FROM auth.users WHERE email = 'revisor@demo.com') THEN

    SELECT id INTO v_user_id FROM auth.users WHERE email = 'revisor@demo.com';

    -- Completar el perfil (el trigger pudo haberlo creado vacío)
    UPDATE profiles SET
      name       = 'Revisor Demo',
      role       = 'member',
      sex        = 'male',
    ...
    -- Sembrar actividad (ON CONFLICT para idempotencia)
    ...
    RAISE NOTICE 'Perfil revisor@demo.com completado y actividad sembrada.';
    RETURN;
  END IF;

  -- -----------------------------------------------------------------------
  -- Caso B: el usuario NO existe todavía. Crear desde cero.
  -- -----------------------------------------------------------------------
  ...
  -- 2. Crear identidad para login con email/contraseña
  SELECT EXISTS (
  ...
  IF v_has_provider_id THEN
    ...
    VALUES (gen_random_uuid(), v_user_id,
      ...
      'email', 'revisor@demo.com', NOW(), NOW(), NOW());
  ELSE
  ...
  -- 3. Completar perfil (el trigger on_auth_user_created puede haberlo creado ya;
  --    usamos ON CONFLICT para actualizar en cualquier caso)
  INSERT INTO profiles (id, email, name, role, sex, weight_kg, height_cm, steps_goal)
  VALUES (v_user_id, 'revisor@demo.com', 'Revisor Demo', 'member', 'male', 78.0, 178, 8000)
  ...
  --    El día de hoy: 6 400 pasos = 80 % del objetivo de 8 000.
  INSERT INTO daily_activity (member_id, activity_date, steps, distance_km, calories_kcal, source, updated_at)
  VALUES
    (v_user_id, CURRENT_DATE - 6, 7823, 5.779, 467, 'manual', NOW()),
    (v_user_id, CURRENT_DATE - 5, 5210, 3.849, 311, 'manual', NOW()),
    ...
    (v_user_id, CURRENT_DATE - 1, 4890, 3.612, 292, 'manual', NOW()),
    (v_user_id, CURRENT_DATE,     6400, 4.728, 382, 'manual', NOW())
  ON CONFLICT (member_id, activity_date) DO NOTHING;
